      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 sm:flex flex-wrap gap-4">
          {allResumes.map((resume, index) => {
            ...
            return (
              <button onClick={()=>navigate(`/app/builder/${resume._id}`)} key={index} 
        ...
        {editResumeId && (
          <form onSubmit={editTitle} onClick={()=>setEditResumeId('')} className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required/>
              <button onClick={()=>setEditResumeId('')} className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors'>Update</button>
              <XIcon className='absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colors' onClick={()=>{
                setEditResumeId('');
                setTitle("")
              }}/>
        ...
        {deleteResumeId && (
          <form onSubmit={deleteResume} onClick={()=>setDeleteResumeId('')} className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              <p>Are you sure you want to delete this resume ?</p>
              <div className='flex items-center justify-end gap-4 mt-3'>
                <button onClick={()=>{
                  deleteResume(deleteResumeId)
                }} className='bg-red-500/70 text-white px-3 py-1.5 rounded hover:bg-red-700 transition-colors'>Delete</button>
                <button onClick={()=>setDeleteResumeId('')} className='text-black-100 px-3 py-1.5 rounded hover:bg-gray-300 hover:text-black-500 transition-colors'>Cancel</button>
              </div>
